...
class ENDTrack_Admin
{
    ...
    public function handle_create_launch()
    {
        ...
        // --- QUEUE AI GENERATION (background, via AJAX polling) ---
        $base_prompt = isset($_POST['ai_prompt']) ? sanitize_textarea_field($_POST['ai_prompt']) : '';

        if (!empty($base_prompt) && !empty($created_pages)) {
            // Map slug_key => AI page type
            $page_ai_types = array(
                'ventas' => 'ventas',
            ...
            // Map slug_key => reference URL from the form
            $page_url_map = array(
                'ventas' => $url_venta,
            ...
            $queue = array();
            foreach ($page_ai_types as $slug_key => $ai_type) {
                if (!isset($created_pages[$slug_key]) || !$created_pages[$slug_key]) {
                    continue;
                }
                $queue[] = array(
                    'post_id' => $created_pages[$slug_key],
                    'page_type' => $ai_type,
                    'ref_url' => !empty($page_url_map[$slug_key]) ? $page_url_map[$slug_key] : '',
                    'prompt' => $base_prompt,
                    'status' => 'pending',
                    'title' => get_the_title($created_pages[$slug_key]),
                );
            }

            if (!empty($queue)) {
                update_option('endtrack_ai_queue', $queue);
                $msg = 'launch_created_ai_queued';
            }
        }
    ...
    public function handle_save_integrations()
    {
        ...
        // Save Claude model
        $allowed_models = array('claude-haiku-4-5-20251001', 'claude-sonnet-4-6');
