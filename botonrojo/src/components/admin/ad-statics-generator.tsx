...
export function AdStaticsGenerator({
  ...
  if (concepts.length === 0) {
    return (
      <p className="rounded-lg border border-white/10 bg-black/30 p-4 text-sm text-zinc-500">
        Regenera los anuncios para obtener conceptos de estático con los que componer imágenes.
      </p>
    );
  }
  ...
  if (mediaItems.length === 0) {
    return (
      <p className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-200">
        Sube primero las fotos del cliente en{" "}
        <Link href="/admin/anuncios" className="underline">
          Anuncios → Biblioteca de fotos
        </Link>
        . Se usan de fondo de los estáticos, tal cual, sin que la IA las retoque.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <form action={generateAction.bind(null, launchId)} className="space-y-5 rounded-lg border border-white/10 bg-black/30 p-4">
        <AiGeneratingOverlay
          messages={["Componiendo el estático…", "Colocando el texto sobre la foto…", "Exportando cada formato…"]}
        />

        <div>
          <div className="text-xs uppercase tracking-widest text-zinc-400">1. Concepto</div>
            ...
            {concepts.map((c, i) => (
              ...
              >
                <input
                  ...
                  className="sr-only"
                />
                <div className="text-sm font-bold text-white">{c.concept ?? `Concepto ${i + 1}`}</div>
        ...
        <div>
          <div className="text-xs uppercase tracking-widest text-zinc-400">2. Foto de fondo</div>
        ...
        <div>
          <div className="text-xs uppercase tracking-widest text-zinc-400">3. Formatos</div>
          ...
          <p className="mt-2 text-xs text-zinc-500">
            Google Search es solo texto — está en la pestaña &ldquo;Google Search&rdquo; de arriba, no
            genera imagen.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-xs text-zinc-500">
            Se usará &ldquo;{selected?.concept ?? `Concepto ${conceptIndex + 1}`}&rdquo; sobre la foto marcada.
          </div>
          <SubmitButton pendingLabel="Generando estáticos…">Generar estáticos</SubmitButton>
        </div>
      </form>
      ...
      {adImages.length > 0 && (
        <div>
          <div className="mb-2 text-xs uppercase tracking-widest text-zinc-400">
            Estáticos generados · {adImages.length}
          </div>
            ...
            {adImages.map((img) => {
              ...
              return (
                <div key={img.id} className="glass overflow-hidden">
                  {img.fileUrl && (
                    <a href={img.fileUrl} target="_blank" rel="noreferrer" className="block bg-black/40">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={img.fileUrl} alt={img.title} className="h-40 w-full object-contain" />
                    </a>
                  )}
