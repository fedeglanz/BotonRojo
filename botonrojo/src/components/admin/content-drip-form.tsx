...
export function ContentDripForm({ launchId, currentStartsAt, contentPageCount, saveAction }: Props) {
  return (
    <form
    ...
    >
      <label className="block">
        <span className="block text-xs uppercase tracking-widest text-zinc-400">
          Inicio del contenido (día 1)
        </span>
        <span className="mt-1 block text-xs text-zinc-500">
          La entrega 1 se abre este día; las siguientes, una por día (hasta {contentPageCount}). Cada
          página muestra el countdown de la siguiente. Vacío = todas abiertas.
        </span>
        <input
          type="datetime-local"
          name="contentDripStartsAt"
          defaultValue={toLocalInputValue(currentStartsAt)}
