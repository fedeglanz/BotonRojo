...
export function RegistroPage({ launch, body }: { launch: Launch; body: RegistroPageBody | null }) {
  ...
  const cardStyle = usableCardStyle(launch.brandPalette, launch.brandDesign?.cardStyle);
            ...
            {body?.bullets && body.bullets.length > 0 && (
              <Reveal className="mx-auto mt-6 max-w-md space-y-2.5 text-left lg:mx-0">
                {body.bullets.map((b, i) => (
                  <RevealItem key={i} className="flex items-start gap-2.5 text-sm lg:text-base">
                    <BrandIcon
                      name="check"
                      size="sm"
                      treatment="plain"
