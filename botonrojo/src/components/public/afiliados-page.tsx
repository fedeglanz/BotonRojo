  ...
  return (
    <main className="relative min-h-screen overflow-hidden">
      ...
      <div className="mx-auto grid min-h-[100svh] max-w-6xl items-center gap-12 px-6 py-16 lg:grid-cols-[1fr_320px] lg:gap-16">
        <div>
          <div className="text-xs uppercase tracking-widest text-[var(--color-accent)]">Programa de afiliados</div>
          <Reveal>
          ...
          <Reveal className="mt-8 grid gap-5 sm:grid-cols-2">
            {paragraphs.map((p, i) => (
              <RevealItem key={i} className="leading-relaxed text-[var(--color-muted-1)]">
                {p}
              </RevealItem>
            ))}
          </Reveal>
        ...
        <Reveal className="lg:sticky lg:top-16">
          <div className="glass p-6 text-center">
            <div className="text-xs uppercase tracking-widest text-[var(--color-muted-2)]">Tu comisión</div>
            <div className="mt-2 font-[family-name:var(--font-mono)] text-5xl font-bold text-[var(--color-red-bright)]">
              {commissionPct}%
            </div>
            {body?.commissionNote && (
              <p className="mt-3 text-sm text-[var(--color-muted-1)]">{body.commissionNote}</p>
            )}
